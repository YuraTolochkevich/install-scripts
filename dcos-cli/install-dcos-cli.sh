...
{ # Show usage information.
  echo "install.sh <installation_path> <marathon_host> [<marathon_port>]"
}
...
curl --silent --fail -O "https://downloads.mesosphere.io/dcos-cli/${DCOS_WHEEL_FILE}"
"$VIRTUAL_ENV_PATH/bin/pip" install --quiet ${DCOS_WHEEL_FILE}
rm ${DCOS_WHEEL_FILE}

# Install the DCOS CLI package
curl --silent --fail -O "https://downloads.mesosphere.io/dcos-cli/${DCOSCLI_WHEEL_FILE}"
